...
| --- | --- | --- |
| Customer booking | The booking page creates the authoritative job through the existing payment/booking path, caches it locally, and syncs it to Supabase. | Booking and notification are separate browser requests, so closing the page after booking can prevent dispatch notification even though the job exists. Move dispatch to a trusted server-side job-created event/outbox. |
...
| Offer notification | Matching contractors receive Resend email and a persisted in-app notification. The portal polls the notification API. | Dispatch is initiated by an unauthenticated browser call and accepts descriptive offer fields from that call. It needs an authenticated/internal job-ID-only command that reloads all offer data server-side, plus delivery/retry tracking. |
