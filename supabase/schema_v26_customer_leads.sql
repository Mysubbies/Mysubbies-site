-- Customer booking-funnel records. Service-role only; browser access is
-- blocked by RLS and capture requires an authenticated customer account.
create table if not exists customer_leads (
  id uuid primary key default gen_random_uuid(),
  booking_job_id text unique,
  customer_id uuid references customers(id),
  name text, email text not null, mobile text, requested_service text, suburb text,
  source text, utm_source text, utm_medium text, utm_campaign text,
  utm_content text, utm_term text, landing_page text,
  stage text not null default 'NEW' check (stage in ('NEW','PRICE_STARTED','PRICE_COMPLETED','QUOTE_REQUESTED','BOOKING_STARTED','BOOKED','LOST')),
  booking_status text, quote_status text,
