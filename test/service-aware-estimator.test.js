...
test('mixed-unit secondary services remain visible with their own unit price', () => {
  assert.match(homepage, /chips\.style\.display = secondaryTasks\.length \? 'flex' : 'none'/);
  assert.match(homepage, /price\.textContent = `\$\$\{Number\(t\.rate\)\.toLocaleString\(\)\} \/ \$\{pricingUnitForTask\(c, t\)\}`/);
  assert.match(homepage, /btn\.dataset\.taskName = t\.name/);
  assert.match(homepage, /return cat\.tasks\.filter\(t => !t\.disabled && !primaryNames\.has\(t\.name\) && t\.name !== companionName\)/);
});

test('visible mixed-unit choices keep independent selection state', () => {
  assert.match(homepage, /function toggleTierSelection\(t, btn\)[\s\S]*selectedTierSet\.set\(t\.name, t\)[\s\S]*renderQtyRows\(\)/);
  assert.match(homepage, /const val = qtyValues\.get\(t\.name\) \|\| \(question\.defaultQuantity \? String\(question\.defaultQuantity\) : ''\)/);
  assert.match(homepage, /const qty = qtyRaw \? parseQuantity\(qtyRaw, pricingUnitForTask\(selectedCat, t\)\) : null/);
});
