...
// Customer and contractor branches are identity-bound through Supabase Auth.
// Query-string emails are retained only as backwards-compatible branch hints;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (all && (customerEmail || contractorEmail)) {
      res.status(400).json({ error: 'all=1 cannot be combined with an account branch.' });
      return;
    }
    if (customerEmail && contractorEmail) {
      res.status(400).json({ error: 'Choose either the customer or contractor branch.' });
      return;
    }
    if (all) {
    ...
    if (contractorEmail) {
      ...
      const { data: ownJobs, error: ownErr } = await supabase.from('jobs')
        .select('full_record, job_number').not('full_record', 'is', null)
        .eq('contractor_email', email).limit(500);
      if (ownErr) throw ownErr;

      const categories = auth.account.categories || [];
    ...
    if (customerEmail) {
      ...
      if (!auth.ok) { res.status(auth.status).json({ error: auth.error }); return; }
      const email = String(auth.account.email).toLowerCase();
      const { data, error } = await supabase.from('jobs').select('full_record, job_number')
        .not('full_record', 'is', null).eq('customer_email', email).limit(500);
      if (error) throw error;
      res.status(200).json({ jobs: (data || []).filter(r => r.full_record)
        .map(r => ({ ...r.full_record, jobNumber: r.job_number })) });
      return;
    }

    // Admin branch; requireAdmin above has already authenticated it.
