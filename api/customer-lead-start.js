...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const { data: priorLead } = await supabase.from('customer_leads').select('stage, booking_status').eq('booking_job_id', jobId).maybeSingle();
    const { data: lead, error } = await supabase.from('customer_leads').upsert({
      booking_job_id: jobId, customer_id: auth.account.id, name: bounded(auth.account.name),
      email: String(auth.account.email).toLowerCase(), mobile: bounded(auth.account.phone),
    ...
    if (lead && lead.stage !== 'BOOKED') {
      const { error: eventError } = await supabase.from('customer_lead_events').upsert(
